...
class TatliDetailVC: UIViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    @IBAction func kaydetClicked(_ sender: Any) {
        if tarifadiText.text! != "" && malzemelerText.text != "" && yapilisiText.text != "" {
            let appDelegate = UIApplication.shared.delegate as! AppDelegate
            let context = appDelegate.persistentContainer.viewContext
            let data = NSEntityDescription.insertNewObject(forEntityName: "TatliTarifleri", into: context)
            ...
            NotificationCenter.default.post(name: NSNotification.Name("tatli"), object: nil)
