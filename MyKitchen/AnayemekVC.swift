...
class AnayemekVC: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    override func viewDidAppear(_ animated: Bool) {
        selectedName = ""
        NotificationCenter.default.addObserver(self, selector: #selector(getData), name: NSNotification.Name("anayemek"), object: nil)
    }
    ...
    @objc func getData() {
        ...
        let getData = NSFetchRequest<NSFetchRequestResult>(entityName: "AnayemekTarifleri")
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let newCell = table.dequeueReusableCell(withIdentifier: "Cell", for: indexPath) as! AnayemekTableViewCell
        newCell.imageAnayemekCell.image = UIImage(data: imageArray[indexPath.row]) //important
        newCell.tarifadiAnayemekCell.text = tarifAdiArray[indexPath.row]
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        performSegue(withIdentifier: "toAnayemekDetailVC", sender: nil)
    }
    
    
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "toAnayemekDetailVC" {
            let destinatonVC = segue.destination as! AnayemekDetailVC
    ...
    func navigationBarLogo() {
        let logo = UIImage(named: "anayemek")
